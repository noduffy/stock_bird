// src/hooks/useChartData.ts (lowDiffAreas 削除版)
...
export const useChartData = (
    ...
    // 3. メインのグラフデータ(monthlyMap)を計算 (変更なし)
    const chartData = useMemo(() => {
        ...
        data.forEach((item) => {
            ...
            const endDepreciationMonth = depreciationEnd
                .add(1, "month")
                .format("YYYY-MM");
            ...
            if (soldMonth) {
                ...
                monthlyMap[endMonthStr]["元金イベント"]!.push(
                    `${item.ビル名}：売却`
                );
                monthlyMap[endMonthStr]["減価償却イベント"]!.push(
                    `${item.ビル名}：売却`
                );
            }
        });
    ...
    // 5. グラフの赤背景エリア (★ lowDiffAreas を削除)
    const { highlightedRanges } = useMemo(() => {
        // "areas" はこの useMemo の中だけで使う
        ...
        // ★ highlightedRanges のみを返す
        return { highlightedRanges: ranges };
    }, [chartData, threshold]);
