// src/pages/GraphPage.tsx (リファクタリング完了版)
...
import { SellBuildingModal, SellFormData } from "../components/graph/SellBuildingModal";
...
const GraphPage = () => {
  ...
  const handleChartClick = (month: string) => {
    ...
    const visibleData = data.filter((b) => {
      ...
      if (sold && currentMonth.isSameOrAfter(dayjs(sold.売却日), "month")) {
        return false;
      }
      return true;
    });
    ...
    // 型が global.d.ts で定義されているので @ts-expect-error は不要
  ...
  return (
    <div className="w-full max-w-[90vw] mx-auto">
      ...
      {/* 6. モーダル (レンダリングされるが非表示) */}
      <AddBuildingModal
        isOpen={showAddModal}
        onClose={() => {
          setShowAddModal(false);
          setEditSimId(null); // キャンセル時も編集IDをリセット
        }}
